...
export const LOCALS_SET_STRING_V1: NodeImplementation = {
  def: {
    nodeType: 'locals.set.string',
    title: '设置局部变量（String）',
    category: 'locals',
    description: '设置 locals 中的 String 变量（params.name）。',
    execInputs: [{ name: 'in' }],
    execOutputs: [{ name: 'out' }],
    inputs: [{ name: 'value', valueType: 'String' }],
    outputs: [],
    paramsSchema: {
  ...
  execute({ node, inputs, runtime }) {
    ...
    const value = canonicalizeChosenValue('String', inputs['value'], node.id);
