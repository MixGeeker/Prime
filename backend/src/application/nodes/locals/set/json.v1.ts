...
export const LOCALS_SET_JSON_V1: NodeImplementation = {
  def: {
    nodeType: 'locals.set.json',
    title: '设置局部变量（Json）',
    category: 'locals',
    description: '设置 locals 中的 Json 变量（params.name）。',
    execInputs: [{ name: 'in' }],
  ...
  execute({ node, inputs, runtime }) {
    ...
    const value = canonicalizeChosenValue('Json', inputs['value'], node.id);
