...
### 4.1 计算时机
- 在 `publish` 成功时计算并落库到 `definition_versions.definition_hash`。
...
然后对得到的对象执行 typed canonicalize（仅对 default/runnerConfig 等可能包含 typed 值的字段）→ JCS → SHA-256。
...
### 5.2 输入选择（MVP 规则）
为了避免 job payload 中的“无关字段”影响 hash，inputsHash **只覆盖 Definition 声明的 variables** + job options：

```json
{
  "variables": {
    "<variable.path>": <canonicalValueOrNull>
  },
...
规则：
- `variables` 的 key 使用 `variables[].path`（见 `GRAPH_SCHEMA.md`），并且必须包含**每一个**声明的 variable：
  - 若 job inputs 缺失该 path：
    - 当 `required=false` 且定义了 `default`：使用 `default`（因此 inputsHash 表示“实际执行用的 effective inputs”）。
    - 否则：值为 `null`。
  - 若存在：按 variable 的 `valueType` 做 typed canonicalize（包含显式 `null` 的处理：`required=true` 时应报 validation error）。
- `options`：
...
> 如果未来需要“hash 覆盖整份 raw inputs（包括未声明字段）”，可以在 schemaVersion=2 加一个开关；MVP 不建议这么做。

### 5.3 计算流程（规范）
1. 从 DefinitionVersion 读取 `variables[]`。
2. 从 job payload 读取 `inputs` 与 `options`。
3. 构造上述 inputsHash 输入对象（variables 全量覆盖，缺失时按 default→null 规则补齐）。
4. typed canonicalize（按 variables 的 valueType + options 规则）→ JCS → SHA-256。
