...
- **Ops（仪表盘）**：查看 definitions/jobs/outbox/DLQ 等运维信息；观察 SDK / 业务模块投递后的引擎状态。
...
3) 启动本前端：默认 `http://localhost:5173`
4) 使用 SDK 或业务模块发送 `compute.job.requested.v1`

在 UI 中可完成：
- Studio：创建/编辑草稿 → 校验 → dry-run → 发布（得到 `definitionHash`）
- Ops：查看 definitions / jobs / outbox / DLQ，定位执行与投递问题
