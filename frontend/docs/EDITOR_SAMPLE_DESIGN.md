...
## 1. 边界
- Studio 只做：建图（nodes/value edges/exec edges）、参数配置、输入契约声明（`flow.start/flow.end/locals`）、调用后端 `validate/dry-run/publish`
- Studio 不做：任何外部 IO（DB/HTTP）；所有业务 facts 都应先由 SDK / 业务模块构建为 flat `inputs`
...
- 数据结构：BlueprintGraph v2（见 `../../doc/GRAPH_SCHEMA.md`），前端类型为 `GraphJsonV2`（`frontend/src/engine/types.ts`）
- UI 元数据：写入 `content.metadata`（引擎忽略且不参与 `definitionHash`）
