...
- 你是运维/值班同学，需要快速定位问题在“前端/后端/worker/MQ/SDK结果消费端”哪一段。

## 先记住：排障的最短路径

1. **先定位在哪一段**：UI 触发？HTTP API？MQ 投递？worker 执行？SDK / 业务模块收结果？  
...
SDK / 业务模块传进来的 `inputs` 没满足图的入口契约（`flow.start` pins）。
...
- worker / SDK 结果消费端的日志片段（出现错误前后 30 行）
