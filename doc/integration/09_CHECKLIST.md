...
- [ ] Provider 侧能处理“发布超时但实际已发布”的情况（确认后再重试）

### B2) 结果事件去重（messageId）

- [ ] Provider 保存已处理 `messageId` 的集合（持久化）
- [ ] 同一个结果事件重复投递时，不会重复落库/重复回调

### B3) 输入校验前置（体验优化）

- [ ] Provider 或 Editor 能在投递前做 inputs 基本校验（缺字段/类型不对尽量早点提示）
...
- [ ] 一份你们自己的“Provider 接入说明”（面向业务方：要填哪些 inputs、输出是什么）
