...
- 你看到“exchange / routingKey / queue”就头大，希望有人用人话解释。
...
> 小白提示：你会在示例 provider（`provider-simulator`）里看到同样的做法：
...
> 小白提示：即使你不设置 `messageId`，系统也能跑；只是你下游会更难保证“重复事件不重复处理”。
